...
%% API functions
%% ====================================================================
-import(string, [len/1, to_upper/1, concat/2, chr/2, substr/3]).
-import(list, [keyfind/3]).

-export ([ enigma/5,setup/5,crypt/2,kill/1,setReflector/1,machine/1,reflectorA/1,reflectorB/1,reflectorC/1,reflectorThinB/1,reflectorThinC/1,setInPlugboard/1,setOutPlugboard/1,inPlugboard/2,outPlugboard/2,keyboardIn/2,lookup/2,lookupback/2,setRotor/3,sequenceRotor/4,ringSetting/4,initialSetting/3,setRurrentRotor/2,increment/1]).

%I have test every component of this work.
%However, I meet some problem in send message between process,
%I don't know how to design a simple system to transfer message.
setup (ReflectorName, RotorSequence, RingSetting, PlugboardPairs, InitialSetting ) ->
    Enigma_PID = spawn(enigma,enigma,[ReflectorName, RotorSequence, RingSetting, PlugboardPairs, InitialSetting]).

enigma (ReflectorName, RotorSequence, RingSetting, PlugboardPairs, InitialSetting ) ->
...
crypt (Enigma_PID, Text) ->
    % spawn ( enigma , keyboardIn , [Enigma_PID,Text]),
...
keyboardIn(Enigma_PID, [CharacterA|CharacterB]) ->
    if 
        ...
        true ->
            % spawn (enigma, keyboardIn, [CharacterA, Enigma_PID]),
            Enigma_PID!
...
setInPlugboard(PlugboardPairs) ->
    ...
    spawn ( enigma , inPlugboard , [ 1, InPlugboard_PID ]).
setOutPlugboard(PlugboardPairs) ->
    OutPlugboard_PID = spawn(enigma, outPlugboard,[]),
    spawn ( enigma , lookup , [1 , OutPlugboard_PID ]).
...
setRotor(RotorSequence,RingSetting,InitialSetting) ->
    RotorCount = 1,
    %io : format("1.~p2.~p3.~p ~n",[RotorSequence,%RingSetting,InitialSetting]),
    InitSetting = tuple_to_list(InitialSetting),
    % io : format("1.~p ~n",[InitSetting]),
    sequenceRotor(RotorSequence,RotorCount,RingSetting,InitSetting).
...
%% ====================================================================
%% Test
%% This is the test program I have designed, it is a test example from 
%% pdf document. It can be used by following command: 
%% 1>c(enigma_test).
%% 2>enigma_test:test().
%% ====================================================================
% -module(enigma_test).
...
%     Result = enigma:crypt(Enigma,"HELLO").
%% ====================================================================
%% Test
%% ====================================================================
